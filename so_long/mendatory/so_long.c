...
/*   Updated: 2025/04/08 19:32:14 by wkannouf         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_map_characters(char *lines_map, t_game *game, int *e, int *p)
{
	...
	while (lines_map[i])
	{
		...
		&& lines_map[i] != 'E' && lines_map[i] != 'C' && lines_map[i] != '\n')
		{
...
static void	fc(char	*lines_map, char *line, int fd)
{
	free(lines_map);
	free(line);
	close(fd);
...
int	check_map(t_game *game, char *path_map, int p, int e)
{
	...
	{
		if (ft_strncmp(line, "\n", 1) == 0)
			return (fc(lm, line, fd), ft_putstr_fd("EMPTY LINE\n", 2), 0);
	...
	if (!game->map)
		return (fc(lm, NULL, fd), ft_putstr_fd("MAP NOT FOUND\n", 2), 0);
	if (!check_map_characters(lm, game, &p, &e) \
	|| p != 1 || e != 1 || game->total_c < 1)
		return (free_map(game->map), free(lm), \
		close(fd), ft_putstr_fd("ERROR!! P != 1 || E != 1 || C < 1\n", 2), 0);
	return (free(lm), close(fd), 1);
}
